"""OrbitExch sport details 发现与解析。

基于 SE (SharpExch) 的 sport/details API 模式重构,替代原有的 DOM 抓取方式。
OE 和 SE 的 API 结构几乎相同,主要差异:
- OE 多了 `marketTypeFilter` 参数
- OE base_url 是 `https://www.orbitexch.com`
"""
...
@dataclass(frozen=True)
class OrbitExchMarketEvent:
    """OE Match Odds market 归一事件。"""
    ...
    market_id: str
    start_ts: int
    runners: tuple[OrbitExchRunner, ...]
    in_play: bool = False


@dataclass(frozen=True)
class OrbitExchSportDetailsRequest:
    """OE `sport/details` 请求描述。"""
...
JsonFetcher = Callable[[OrbitExchSportDetailsRequest], dict | Awaitable[dict]]
...
class OrbitExchDiscoveryClient:
    """OrbitExch 发现客户端。
    ...
    async def discover_events(self, sport_configs: Iterable[Any] | None = None) -> list[OrbitExchMarketEvent]:
...
def events_from_sport_details(
    ...
    *,
    target_competitions: Iterable[str] | None = None,
) -> list[OrbitExchMarketEvent]:
    """解析 OE `/customer/api/sport/details` 响应。
...
def _parse_runners(raw_runners: list, home_team: str, away_team: str) -> list[OrbitExchRunner]:
    parsed: list[OrbitExchRunner] = []
