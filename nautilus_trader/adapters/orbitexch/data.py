...
class OrbitExchDataClient(LiveMarketDataClient):
    ...
    def _clear_competition_order_books(self, page_key: str) -> None:
        ...
        if cleared:
            self._log.warning(
                f"OE competition page {page_key} prices disconnected; cleared {cleared} cached order book(s)",
            )
        ...
        for runner in parsed.get("runners", []):
            ...
            for instrument_id, claim in routing.get(sel_id, []):
                ...
                if deltas is not None:
                    ...
                    binary_market_id = str(info.get("binary_market_id") or "")
                    members = self._market_order_book_members.get(binary_market_id)
                    market_subscribed = members is not None and instrument_id in members
