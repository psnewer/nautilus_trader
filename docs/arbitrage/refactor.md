...
| 2026-07-15 (#231) | **review 严重项 #1/#2 收口:decimal LAY liability + 3-way 校验原子发布**。① Venue Registry 新增 `order_liability`,Risk 下单前余额门控与 Execution `OrderAccepted` 本地预扣共用同一经济公式:probability=`qty×price`,decimal BACK=`qty`,decimal LAY=`qty×(odds−1)`;修复 LAY 高赔率订单按 stake 低估占用的问题。② 3-way matching candidate 按同一拆分批次组成 validation group;单 role PASS 只记状态并退订临时 OBD,home/draw/away 全部 PASS 后才统一 register+publish;任一 role FAIL 前 Strategy 不可能看到早到兄弟。缺 role/不足两 tradable venue 的 3-way 整批不发布。定向测试覆盖 LAY 10@5 liability=40、全 PASS 原子发布、先 PASS 后 sibling FAIL 零发布。#230 已解决 review #3/#4;subscription eviction(#5)按用户要求不在本次处理。 |
| 2026-07-15 (#230) | **`[yes,no]` recovery + Risk profit gate 共用 claim/side 感知净持仓经济投影(已落地)**。#229 显式留下两个缺口表面上分属 Strategy/Risk,根因相同:Portfolio 与 recovery 都丢弃 NT `Position.side`,把 `abs(quantity)` 按 instrument role 和 BACK 公式计算;但 OE/SE no 真单经执行重定向后是 yes instrument 上的 SHORT/LAY。定论并实现:单腿 position outcome/金额公式归 Venue Registry 横切真理源(venues §4.1),提供 `outcome_for_position` + `leg_economics`;PM NO token LONG 直接归 no,decimal SHORT 归二元 complement,lay size q@L 的 share/profit/loss=`qL/q/q(L−1)`。Portfolio、share limit、snapshot recovery 共用;RiskEngine profit gate 判断本身不改。Recovery 已移除 `[yes,no]` bail,decimal no candidate 继续透传到现有 SELL@lay。边界:只计算 NT 当前净 Position,不从 OE/SE 净额还原已平仓历史 realized PnL。定向验证:`tests/arbitrage/common/test_venues.py` + `tests/arbitrage/risk` + `tests/arbitrage/strategy`。 |
