...
| Q20 ✅ | **已由 #266 修订**:删除全状态 `OpportunitySnapshot`;Strategy 随用随读 live Cache,只记录 pair open-order digest,Execution barrier release 前统一比较 | strategy §4.5 / execution §3.5 |
| Q21 ✅ | **已由 #267 修订**:scope-priority、套利/补偿双树、Condition 嵌套与 AND/OR/NOT 保留；`self_hits` 叶子改为无状态 `StateQuery` 直接查询 `EvalContext`，SignalStore/persistent/transient/pre-match 派生退役。详细设计 `architectures/strategy/architecture.md` | §5.4 / Step 4 |
...
| 2026-07-23 (#266) | **撤销 Q20 全状态快照,改为 Execution release 前校验 pair open orders**。原因:快照既不能阻止评估到真实提交之间的新成交,又让 Strategy 长期维护一套 Cache 复制语义;真正需要阻断的是该窗口内挂单事实变化。定夺:Strategy 对行情/持仓/instrument 随用随读 live Cache,只在 evaluation 开始记录 pair-wide open-order digest 并透传所有真实腿;Execution barrier 收齐腿后统一重算,不一致整组拒绝。保持既有 residual cancel-only 优先;不增加 fill epoch,接受 open-order 集合恢复原值的 ABA 边界。详细契约见 strategy architecture §4.5、execution §3.5、common §1、synchronization §8.4bis;测试见 strategy/execution README。 |
