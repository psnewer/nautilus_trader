...
- ✅ **#39/#91 `Debug{PM,OE,SE}DataClient`(Q11.A)**(`src/arbitrage/debug/data_clients.py`)— `_DebugDataClientMixin` 拦 `_handle_data`,`_maybe_substitute(data) → data|None`;PM/OE/SE data factory 读 `ArbContext.debug_config` 分支(`enabled` → 装 Debug 子类)。#91 起内置最小 `MockCategory.ODDS` → NT `OrderBookDeltas` 替换:conditions 可按 `instrument_id` / `venue` / `market_type` / `selection_role` 匹配;`data.bid|back` 生成 BUY 档,`data.ask|lay` 生成 SELL 档,输出 snapshot `CLEAR + ADD`。更复杂场景仍通过用户子类化覆盖 `_maybe_substitute`。**SE 严格对齐 OE**。
- ✅ **#40/#93 `SkipExecution{PM,OE,SE}Client`(Q11.3)**(`src/arbitrage/debug/execution_clients.py`)— `is_override_active("skip_execution")` 真时:`_submit_order` **保留 `_begin_session` / `execution.started/finished` / per-pair gate 生命周期**,只跳真 venue 上送;随后 `generate_order_accepted` + `generate_order_filled` mock 全成交(PM=USDC_POS / OE=USD / SE=USD,commission=0,liquidity=TAKER,venue_order_id=`MOCK-{cid}`)。`_begin_session` 返回 False(cancel-only / residual open order)时不 mock fill。**`_cancel_order`/`_cancel_all_orders`/(OE/SE)`_cancel_residual_one` no-op**(mock 单已终态全成,且不可拿 MOCK id 去真 venue 撤)。未激活透传 super。**SE 严格对齐 OE**(同为 exchange-style venue,quote=USD,browser-based 连接)。支持 `timeline` mock 配置实现部分填/拒单/撤单时序(Q11.4)。
...
|---|---|---|
| Strategy | ❌ 撤回 | 参数化 first-class 吸收,不需要 Debug 子类 |
| Risk | `DebugArbitrageLiveRiskEngine` | `skip_check_size`:`_check_order` 子类覆盖,跳过 NT 父类最小限额检查 |
| Execution(PM) | `SkipExecutionPolymarketClient` | `_submit_order` 进入 session 后 mock fill(不真正下单) |
| Execution(OE) | `SkipExecutionOrbitExchClient` | 同 PM,quote=USD |
| Execution(SE) | `SkipExecutionSharpExchClient` | **严格对齐 OE**,quote=USD |
| Execution(settlement) | (子类/开关) | `skip_settlement`:mock `TxResult` |
...
```
src/arbitrage/debug/
  config.py             # DebugConfig + MockCategory(普通对象,DI 注入)
...
- [x] `DebugConfig` + `MockCategory`(含 TIMELINE)
