...
- [x] **slice 7A**:OE data factory 真接 scraper + Provider aliases 注入 ✅ #46(+9 passed);当时 PM 6-key + sport 过滤留后续 slice,现已由 `ArbPolymarketInstrumentProvider.load_all_async` series-based discovery 收口;**known divergence**:`OrbitExchScraper` 自管 browser 不走 BrowserManager(Q2 原意只覆盖 Data+Exec,discovery 是第三方 + unauthenticated 够用)
