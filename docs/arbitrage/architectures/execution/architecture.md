...
- **已确认**:帧 envelope + 分型 + BALANCE schema(`tests/arbitrage/adapters/orbitexch/test_ws_general_frames.py` 8 passed)。**仍是工作假设**:`CURRENT_BETS` 的**单 bet item 字段**(抓到的样本为空 `[]`)——假设与 REST `/customer/api/currentBets` 的 `bets[]` 同源(`marketId`/`selectionId`/`sizeMatched`/`averagePrice`/`side`/`profitNet`/`liability`,参旧 `load_orbitexch_bets`),待 populated 抓帧确认。
```python
...
- [ ] **live seam**:`_connect`(browser/executor/general WS 接线)、`_place_via_executor`(NT Order→executor 旧 Order:market_id/selection_id 取自 instrument.info)、`_cancel_order`/`_cancel_all_orders`/`_cancel_residual_one`(executor)
- [ ] **待 `CURRENT_BETS` populated 抓帧**:`_on_current_bets` 单 bet item→`generate_order_*`、`generate_*_status_reports`(现 stub 返 [] 让 client 可连);OE 健康检查页面 reload(宿主 DataClient,另落)
...
> **待 Step 5 核实**:OE `CURRENT_BETS` 单 bet item 字段(待 populated 抓帧);merge/redeem 频率是否要低于健康检查 tick。
