...
`SetRiskParamsCommand` 使用 `None` 表示不覆盖该字段;当前热字段包括 `match_tp` / `match_sl` / `min_probability` / `max_probability` / `prob_buy_only`，最后一项必须是 boolean。`SetArbitrageParamsCommand` 承载普通套利运行默认值 `share` / `max_leg_share` / `fx` / `evaluate_on_depth_change`;最后一项由 StrategyEvaluator 决定纯深度 OBD 是否唤醒评估。这些字段不归 Risk 配置所有。
