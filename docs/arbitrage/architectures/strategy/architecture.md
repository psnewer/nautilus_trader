...
- `outcomes: list[str]`(#228,已落地 2026-07-15)— 该 pair 的互斥 outcome 集,2-way
  `["home","away"]`、3-way 拆分 pair `["yes","no"]`。**builder 从腿的 claim 派生**(任一腿
  info 带 `claim` → `[yes,no]`,否则 `[home,away]`;与 `MatchedPair.outcomes` 按构造一致,
  snapshot 自足、不依赖 evaluator 留存 MatchedPair 状态)。
  Check 给腿打 outcome 标签的统一约定:`info.get("claim") or info.get("selection_role")`
  (3-way 腿含 yes 腿全部显式带 `claim`,2-way 腿一律不带 → 一行表达式覆盖两类 pair);
  合法标签集合 = `snap.outcomes`,不再硬编码 role 组合。claim=no 的候选腿另带
  `lay_price`(= no book ask 原值)与 `exec_instrument_id`(执行重定向,data §3.1b),
  place_bets 的 submit spec 用 `leg.get("exec_instrument_id") or leg["instrument_id"]`。
  **mean_rebate_recovery 对 `[yes,no]` pair 显式 bail(return False)**:no 侧敞口以
  SHORT/lay 头寸存在,share/缺口归属需要 claim 感知的持仓核算,另行设计,未落 #228 范围。
...
| 类 | 文件 | 用 |
|---|---|---|
| `pre_match` self_hits signal | `StrategyEvaluator` | Evaluator 每轮从 snapshot 写 `ctx.store["pre_match"] = not snapshot.in_play`,strategy JSON 用 `"self_hits": {"signal": "pre_match"}` 做 condition 级门控。`pre_match` 不再注册为 Check 类型,配置若写进 `checktion` 会 fail-fast |
| `MeanRebateCheck(min_rate, share=None)` | `src/arbitrage/strategy/checks/mean_rebate.py` | 平均返水套利算法:按 outcome 标签(#228:`info.get("claim") or info.get("selection_role")`,合法集合 = `snap.outcomes`;此前为硬编码 `home/away[/draw]` role 组合——**三 role 分支随 #228 退役**,3-way 改为每 market 内 yes/no 套利×3 个 pair)分组 → 从 NT `InstrumentId.venue` / 兼容字符串提真实 venue → 各 venue 取 best_ask → 经 Venue Registry `probability_from_price(venue, price, claim)` 转概率(decimal venue `claim=no` 腿为 `1−1/price`,其余 `1/price`;probability venue 原值)→ 每个 outcome 取 min → sum → `rate = 1 - sum`;同概率 tie-break 经 Venue Registry `venue_preference_rank` 稳定排序;`>= min_rate` 时写带 `share_if_wins` 的 `ctx.scratch["legs"]` + return True(`claim=no` 腿的 `price` 即 no book 的 ask=lay 原值,直通 place_bets 的 SELL@lay 转换)。`share` 未显式配置时读 `ctx.strategy_defaults["share"]`。"每 outcome ≥2 条可比腿"规则保持:某 venue 某 outcome 缺报价(如 lay 侧空)= 该腿本轮不存在,组内候选不足则本轮无机会,OBD 更新后自愈 |
| `OneSideRebateCheck(min_rate, share=None)` | `src/arbitrage/strategy/checks/one_side_rebate.py` | 定向返水候选生成:按 outcome 标签(#228:同 mean_rebate 的 `claim or selection_role` 约定 + `snap.outcomes` 合法集)收集所有可买 leg(同 outcome 多 venue 都保留)→ 枚举每个 outcome 选一条 leg 的笛卡尔积 → 对每个组合枚举 target outcome → `rate=(1-total_prob)/target_prob`;达阈值时写 `ctx.scratch["candidates"]`,每个 candidate 的 legs 已带 USD 口径 `qty/share_if_wins/cost`。qty 通过 Venue Registry `qty_from_share(venue, share, price)` 计算;`share` 未显式配置时读 Web 默认 |
...
| `MeanRebateRecoveryCheck(min_repaired_rebate=-0.05)` | `src/arbitrage/strategy/checks/mean_rebate_recovery.py` | mean_rebate 补救检查:从 USD 口径 `snapshot.positions` 计算每个 outcome 的实际 share(#228:outcome 归属同用 `claim or selection_role` 标签),目标 `target_share=max(actual_share_by_outcome)`;对缺口 outcome 取当前 best ask 最便宜 venue,同概率 tie-break 经 Venue Registry `venue_preference_rank`;写只包含缺口的 `ctx.scratch["legs"]` 且每 leg 带最终 USD 口径 `qty`;qty 与实际 share / return rate 公式通过 Venue Registry odds_model 派生;补齐后的最差 rebate 必须 `>= min_repaired_rebate` |
...
| `PlaceBetsAction(price_overrides=None, qty_overrides=None, intent="arbitrage")` | `src/arbitrage/strategy/actions/place_bets.py` | 通用下单:consume `ctx.scratch["legs"]`;不决定原始 share,也不接收 `share` 参数。默认买入路径 qty 优先级为 `qty_overrides` > `leg["qty"]` > `leg["share_if_wins"]` 经 Venue Registry `qty_from_share` 推导;若 `leg["claim"]=="no"` 且 venue capability 为 decimal odds,最终 submit spec 转为 `side=SELL`、`price=leg["bid"/"best_bid"/"lay"/"lay_price"]`、`qty=share_if_wins/price`(除非显式 `qty_overrides`),即用 lay 表达买 NO。该转换只看 `is_decimal_odds_venue`,不硬编码 OE/SE,也不改变 anchor outcomes 定义;非 decimal venue 的 `claim=no` 仍走普通买入路径。若 leg 缺 `qty/share_if_wins`、decimal no 缺 lay 价格,或明确 `tradable=false` / `anchor=true`,整次机会 abort,避免半边、0 size 或 PMSPORTS anchor 下单。`price_overrides` / `qty_overrides` 是 venue-keyed Action 参数,只改最终 submit spec,不改 Check 用真实 order book 选腿;`intent` 写入最终 submit spec,submitter 转成 NT `Order.tags=["arb:intent=<intent>"]`,供 Risk 区分普通套利与补救单;`ctx.submitter` 存在时提交 NT `SubmitOrder`,否则 log-only fallback |
...
- size 计算优先级:默认买入路径为 `qty_overrides[venue]` > `leg["qty"]` > `leg["share_if_wins"]` 经 Venue Registry `qty_from_share` 推导。decimal odds venue 若 `leg["claim"]=="no"`,`place_bets` 在最终 submit spec 处把订单改为 `SELL @ lay/bid`,并用 `share_if_wins / lay` 重算 qty,避免沿用 ask/back 下的旧 qty;显式 `qty_overrides` 仍最高优先级。这样 `mean_rebate`/`one_side_rebate`/`mean_rebate_recovery` 这类 Check 均必须产出完整计划腿,继续复用 `place_bets`,不另起专用 Action。第一阶段 SE 接入中,`SHARPEXCH` 与 `ORBITEXCH` 由 registry 标记为相同 decimal odds 模型。
...
**框架基础(纯逻辑,可全单测)**:
- [x] `SignalStore`(`signals.py`)+ 双状态读写测试(`test_signal_store.py` / `test_signal_store_view.py`)
- [x] `BoolExpr` / `SignalRef` / `AndExpr` / `OrExpr` / `NotExpr`(`bool_expr.py`)+ AND/OR/NOT 嵌套求值测试(`test_bool_expr.py` / `test_json_loader.py`)
...
- [x] `OpportunitySnapshot` 取数(`snapshot.py`)+ pre_match signal 派生 + PMSPORTS anchor 不进快照测试(`test_snapshot.py` / `test_evaluator.py`)。当前实现不持有长存 snapshot dict,回收靠 per-evaluation ctx 作用域。
