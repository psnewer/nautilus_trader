...
| `TrendGateAction(up=True, complement=False)` | `src/arbitrage/strategy/actions/trend_gate.py` | 读取 live `PairPriceStore.trend_price` 与当前各 outcome 跨 venue 最低 best-ask 隐含概率，逐 outcome 直接比较：`current > baseline` 为 up，`current < baseline` 为 down，相等为 flat。`up` 缺失/`True` 留 up，`False` 留 down。`complement` 缺失/`False` 时各 outcome 仍独立过滤；显式 `True` 时，完整二元 outcome 同为 up 或同为 down 会使本轮全删，包含 flat 时互补门不拦截，之后仍按 `up` 过滤。它不恢复相邻帧 momentum、各 venue 同向或要求严格一 up 一 down，也无 `steps/trend` 参数。输入按 `selected_candidate` → `candidates` → legs-only 处理：已选 candidate 同步回写其 legs 与 `scratch["legs"]`；未选择的候选池逐 candidate 过滤 legs、淘汰空 candidate，并保留 rate 等元数据，供后续 `candi_select` 对过滤结果做门控与选择；mean/recovery 的 legs-only 则直接回写 `scratch["legs"]`，不凭空构造 candidate。基准、当前完整向量或 outcome 缺失时 fail-closed 全删。撤单 candidate 不处理。详见 §3.8.3 |
...
判定：`current > baseline` 为 up，`current < baseline` 为 down，相等为 flat；默认不使用相邻帧 momentum、
跨 venue 方向一致、互补 outcome 反向推断或累计 `steps`。`trend_gate(complement=true)` 只拦截
二元 outcome 同为 up 或同为 down；up/down、up/flat、down/flat 与 flat/flat 均不被互补门拦截，
随后仍由 `up` 参数筛选目标方向。缺基准或不完整数据时 fail-closed。
