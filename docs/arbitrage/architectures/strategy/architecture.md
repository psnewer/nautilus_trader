...
- Action ACK 收口参数(#298):`enable_timeout` 必须是 JSON boolean，缺失/`false` 保持
  submit session 等待全成、失败终态或 watchdog 的既有行为；`true` 时
