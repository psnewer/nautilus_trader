...
- **链路精简到 `[place_bets]`**:pre_rebate 单腿、按 `share` 精确买、纯 PM,不需要
  ...
  **代价**:candi_select 的最小下注门控被去掉 → 由 **Risk 侧通用 `min_notional/min_quantity` 与 PM BUY-only `min_buy_quantity/min_buy_notional` 兜底**;
