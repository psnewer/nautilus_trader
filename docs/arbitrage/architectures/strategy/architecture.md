...
- `outcomes: list[str]`(#233)— 所有 binary pair 固定为 `["yes","no"]`。builder 自足,
  ...
  #230 已移除 recovery 对 `[yes,no]` pair 的 bail;已有 Position 与候选腿均按上述 outcome
  集合归属,持仓经济量统一调用 venues §4.1 的 claim/side 感知 helper。
...
| `MeanRebateCheck(min_rate, share=None)` | `src/arbitrage/strategy/checks/mean_rebate.py` | 按 canonical `claim=yes/no` 分组并以 `snap.outcomes` 校验完整性；每个 outcome 取跨 venue 最低隐含概率后求 rate。decimal 概率换算读 `quote_claim`:真实 runner 为 `1/price`,合成 no 腿为 `1−1/price`。只有带 `exec_instrument_id` 的候选透传 lay 执行字段。阈值、tie-break、share 默认和“每 outcome 至少两条可比腿”规则不变 |
...
| `MeanRebateRecoveryCheck(min_repaired_rebate=-0.05)` | `src/arbitrage/strategy/checks/mean_rebate_recovery.py` | mean_rebate 补救检查:从 USD 口径 `snapshot.positions` 计算每个 outcome 的实际 share,目标 `target_share=max(actual_share_by_outcome)`;对缺口 outcome 取当前 best ask 最便宜 venue,写只包含缺口的 recovery legs;补齐后的最差 rebate 必须达标。#230 已将 position outcome/金额统一委托 venues §4.1 并支持 `[yes,no]`;候选按 `claim or selection_role` + `snap.outcomes`,decimal no leg 透传 `claim/lay_price/exec_instrument_id` 复用 `PlaceBetsAction` 的 SELL@lay |
...
| `PlaceBetsAction(price_overrides=None, qty_overrides=None, intent="arbitrage")` | `src/arbitrage/strategy/actions/place_bets.py` | 通用“语义腿→实际订单”边界。qty 优先级不变。只有 `exec_instrument_id` 指向另一真实 instrument 的 decimal 合成腿才转 `SELL@lay`;真实 instrument 即使 `claim=no` 仍正常 BUY/BACK。probability BUY 读取本轮 snapshot 的互斥 LONG 仓位,优先展开为 SELL 减仓 + BUY 剩余量。拆单使用快照冻结的 `min_quantity/min_buy_notional/size_increment`:SELL 子单只满足 `min_quantity`,BUY 子单同时满足 `min_quantity` 与 `quantity×price>=min_buy_notional`;无法形成合法子单则回退原 BUY，交最终 Risk 判断。**decimal venue(OE/SE)市价单开关(#256 续)**:`_resolve_side_and_price` 算出的限价之后,若 `get_arb_context().market_order_enabled` 为真(execution 配置段 `market_order_enabled`,on_restart)且该 venue 是 decimal odds,用 `leg["instrument_id"]`(quote_legs 定价所用的原始 instrument,不是 `exec_instrument_id` 提交目标)对应 book 的 `worst_ask`(最深档隐含概率)经 `price_from_probability`(与 quote_legs 同一 `quote_claim`)换算出的最差价覆盖限价,保证成交而非最优价;显式 `price_overrides` 优先于市价单;查不到深度(缺该 instrument 的 book、或 book 空)时退回原限价,不报错。全部转换后才生成实际 `leg_key/expected_legs`,并把 per-venue 整组资金需求写入 `venue_required_balance`;汇总逐单资金需求时统一调用 Venue Registry `order_required_balance`。不做 FX、不 live 重读、不修改 barrier |
...
语义:
- `mean_rebate_recovery` 只负责判断并生成补缺口 legs:目标 `target_share = max(actual_share_by_outcome)`,只对 `missing_share > 0` 的 outcome 写 leg。#230 已落地:existing position 通过 venues §4.1 的 claim/side 感知投影归属 outcome 并计算 share/return;候选按 `claim or selection_role`、合法集合取 `snapshot.outcomes`。decimal no 候选写 `claim=no/lay_price/exec_instrument_id`,最终由现有 `place_bets` 转 SELL@lay;PM no 候选仍 BUY NO token。补救 qty 继续经 `qty_from_share(venue, missing_share, price)` 推导。
