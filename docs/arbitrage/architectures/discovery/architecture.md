...
最小下单元数据契约:PM 实盘拒单证明 `minimum_order_size` 对 BUY/SELL 两侧都生效，解析层写入 `BinaryOption.min_quantity`(当前通常 5 shares)，由 Strategy 与 NT Risk 通用 quantity 门共同消费；PM 的 1 USD 下限仍只约束 BUY，写入 `info["min_buy_notional"]=1.0`。另写 `info["order_size_increment"]=0.01` 供 submitter 将两侧下单量保留到两位网格：SELL 向下取整，BUY 正常四舍五入。OE 的 venue 最小值是 **stake 7 GBP**,但 adapter 外部 OE `quantity` 统一按 USD stake 解释,因此 Provider 写入 `BettingInstrument.min_notional = Money(7 * arbitrage.fx, USD)`。
...
> **交易最小值(#346，修订 #344)**:Gamma/CLOB 归一化字段 `minimum_order_size` 是两侧 share 下限。Provider 写通用 `BinaryOption.min_quantity`(当前默认/实盘为 5)，不再写 `info["min_buy_quantity"]`；BUY 还要求 `quantity × price >= 1 USD`，写入 `info["min_buy_notional"]=1.0`。Strategy 与 NT Risk 对 BUY/SELL 都应用 share 下限，仅 BUY 应用金额下限。
...
- **OE/SE**(`adapters/{orbitexch,sharpexch}/data.py`):`_send_all_instruments_to_data_engine()`(`provider.get_all()`→`_handle_data`)+ `_update_instruments(interval)` task(**直调 `load_all_async`**,Gap-α-proof)+ `_connect` 首抓 + `_disconnect` cancel;config `update_instruments_interval_mins`(默认 60)。两者都不登录、不持登录锁,并等待 execution 登录写入共享 BrowserContext 的 `CSRF-TOKEN`(最长 `config.page_timeout`)。OE 随后用 context request API 请求 `sport/details`;SE 因 Cloudflare/TLS 对 APIRequestContext 与真实 page 的处理不一致,每轮创建一张临时 `se-discovery` page,首次导航同源 `sport/details` API URL 建立稳定 origin并等待 challenge 自动结束(不加载 `/customer` SPA),全部 POST 分页复用 page-native fetch,最后关闭。启动首轮若 CSRF/页面/请求失败,记录 warning 并保留周期 task 等下一轮。
