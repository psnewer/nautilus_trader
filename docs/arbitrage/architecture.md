...
| **横切·订单簿源帧** | 上游源市场帧原子更新 + 二元策略市场事件边界 | **✅ 详细** | [_cross-cutting/order-book-frame.md](architectures/_cross-cutting/order-book-frame.md) |
...
- **订单簿源帧**：区分 venue `source_market_id` 与策略 `binary_market_id`，DataEngine 先应用整条上游帧再逐二元市场发布 → `order-book-frame.md`
