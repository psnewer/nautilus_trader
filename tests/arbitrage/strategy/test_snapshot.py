...
class _FakePortfolio:
    def __init__(self, rebate=None):
        self._rebate = rebate or {"home": 0.10, "away": 0.05}

    def way_rebate(self, pair_id, account_id=None):
        return dict(self._rebate)
...
def test_build_snapshot_collects_pair_state():
    ...
    snap = build_snapshot("match_X", cache=cache, portfolio=_FakePortfolio(), pair_registry=pair_registry)
    ...
    assert snap.way_rebate == {"home": 0.10, "away": 0.05}


# ── snap.2: 快照内容不被快照后的 cache 更新扰动(关键不变量)──
def test_snapshot_contents_isolated_from_subsequent_cache_changes():
    """snap.2: build_snapshot 返回后,cache 新写入不影响 snapshot.order_books / positions / way_rebate。
    ...
    portfolio = _FakePortfolio(rebate={"home": 0.10})

    snap = build_snapshot("match_X", cache=cache, portfolio=portfolio, pair_registry=pair_registry)
...
def test_unknown_pair_id_yields_empty_snapshot():
    ...
    snap = build_snapshot("missing", cache=cache, portfolio=_FakePortfolio(), pair_registry=pair_registry)
    assert snap.instrument_ids == [] and snap.order_books == {} and snap.positions == []


# ── 边界:portfolio 无 way_rebate 接口 → snapshot 给空 dict 不抛 ──
def test_portfolio_without_way_rebate_safe():
    cache = _FakeCache()
    pair_registry = PairRegistry(); pair_registry.register("p", ["X.PM"])
    cache.set_book("X.PM", "b")
    snap = build_snapshot("p", cache=cache, portfolio=SimpleNamespace(), pair_registry=pair_registry)
    assert snap.way_rebate == {}
...
def test_snapshot_in_play_false_when_no_instrument_marks_inplay():
    ...
    snap = build_snapshot("p", cache=cache, portfolio=_FakePortfolio(), pair_registry=pair_registry)
