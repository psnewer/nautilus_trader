...
- ✅ `test_check_current_rebate.py` + `scenarios/one_side_rebate`:one_side_rebate 套利树读取当前 Portfolio outcome 净利润/share，以最大 outcome share 为共同分母；覆盖默认阈值 0、任一 outcome 低于阈值、空仓按 0、outcome 不完整、缺 Portfolio、经济投影异常，以及 AND 失败时回滚已生成 candidates
- ✅ `test_check_cross_venue.py`:套利树 checktion 过滤全同 venue 的 `legs`;对 `candidates` 数组删除全同 venue candidate,剩余为空则拒绝;补偿树不使用该 check
- ✅ `test_check_mean_rebate_recovery.py`(9):已有单边持仓 → 生成缺口 outcome recovery leg 到最大实际 share / 修复后最差 rebate 低于阈值不触发 / 无缺口不触发 / OE/SE 缺口 qty 与实际 share 经 Venue Registry 按 USD stake gross payout 反算(`missing/odds`,不乘 fx) / 同概率 tie-break 经 Venue Registry `venue_preference_rank` / typed `InstrumentId` info map 兼容 / 既有持仓 `avg_px_open=0` 时不触发 recovery / `venue_select=True` 时即便 OE 赔率更优也只选 PM 补救腿、缺口 outcome 无 PM 报价则 fail-closed 不补
