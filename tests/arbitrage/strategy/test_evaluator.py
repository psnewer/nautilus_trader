...
@pytest.mark.asyncio
async def test_matched_pair_routes_directly_with_embedded_pair_id():
...
@pytest.mark.asyncio
async def test_no_strategy_mounted_no_op():
    actor, store, pair_reg, strat_reg, loop, _ = _harness()
...
@pytest.mark.asyncio
async def test_execution_active_skips_evaluation():
...
@pytest.mark.asyncio
async def test_arb_hit_blocks_comp_action():
    actor, store, pair_reg, strat_reg, loop, _ = _harness()
    arb_action = _RecordingAction("arb")
    comp_action = _RecordingAction("comp")
    strat_reg.register_pair("match_X", _strategy(True, True, arb_action=arb_action, comp_action=comp_action))
...
@pytest.mark.asyncio
async def test_comp_hit_when_arb_miss_fires_comp():
    actor, store, pair_reg, strat_reg, loop, _ = _harness()
    arb_action = _RecordingAction("arb")
    comp_action = _RecordingAction("comp")
...
# ── 边界:两边都 miss → 无 fire ────────────────────────────────────
@pytest.mark.asyncio
async def test_both_miss_no_fire():
    actor, store, pair_reg, strat_reg, loop, _ = _harness()
...
@pytest.mark.asyncio
async def test_signal_collector_called_before_evaluation():
    collector_calls = []
...
@pytest.mark.asyncio
async def test_submitter_wired_into_eval_context():
...
@pytest.mark.asyncio
async def test_non_routable_data_silently_skipped():
    actor, store, pair_reg, strat_reg, loop, _ = _harness()
...
@pytest.mark.asyncio
async def test_matched_pair_subscribes_obd_deduped():
