...
**✅ 已接入 reports 2026-06-15;CURRENT_BETS 前置收紧 2026-07-09;上层 liveness 裁决 2026-08-04**。前置:`_FakePageReload`(记 reload 次数 + 可选 on_reload 模拟 CURRENT_BETS 重推)。用例:① 已有 CURRENT_BETS 且 WS 新鲜(`_mark_exec_frame` 后)→ `_ensure_exec_snapshot_fresh` 不 reload(reload_count==0);② WS 新鲜但从未收到 CURRENT_BETS → reload + CURRENT_BETS 重推 → True;③ 陈旧 → reload + CURRENT_BETS 重推 → True(reload_count==1);④ reload 后 CURRENT_BETS 不重推 → `_reload_exec_page` 超时(`_reload_bets_wait_ns` 调小)→ adapter 抛异常，由启动/周期 reconciliation 上层标 dead;⑤ 两并发 `_ensure_exec_snapshot_fresh` → single-flight 只 reload 一次;⑥ adapter 的单/复数 report 方法自身不写 liveness;⑦ reload 成功 → 正常返回，由上层标 alive。**验收**:`test_orbitexch_client.py::test_ensure_fresh_skips_reload_when_current_bets_and_ws_fresh` / `test_ensure_fresh_reloads_when_ws_fresh_but_no_current_bets` / `_reloads_when_stale_and_succeeds` / `test_reload_exec_page_timeout_returns_false` / `test_ensure_fresh_single_flight_one_reload` / `test_reconcile_reports_without_current_bets_does_not_write_oe_liveness` / `test_reconcile_reports_stale_snapshot_reload_failure_does_not_write_liveness` / `test_reconcile_reports_stale_snapshot_reload_success_stays_alive`。
