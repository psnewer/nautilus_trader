...
def test_data_factory_constructs_provider_when_discovery_config_present(monkeypatch):
    ...
    bootstrap.prepare_arb_context(
        se_discovery_config=se_venue,
        se_sport_aliases={"Tennis": "Tennis"},
        se_competition_aliases={"Men's Wimbledon 2026": "ATP"},
    ...
    assert bootstrap.get_arb_context().se_instrument_provider is provider.return_value
...
def test_data_and_exec_factories_share_browser_manager(monkeypatch):
    ...
    assert exec_client._browser_lock is bootstrap.get_arb_context().se_browser_lock
...
def test_exec_factory_create_with_context_returns_client():
    ...
    assert client._browser_manager is bootstrap.get_arb_context().se_browser_manager
