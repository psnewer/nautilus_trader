...
| `test_execution_ack.py` | **#253** PM 下单回执即 ack:8 tests —— `_mark_accepted_emitted` 去重(首次 True / 重复 False / 有界挤出最老)、`_post_signed_order` 成功回执 `generate_order_accepted` 一次(WS 先 ack 则跳过;失败只 reject 不 accept)、`_process_batch_response` 同语义、WS PLACEMENT 对 SUBMITTED 单 ack / HTTP 已 ack 则幂等跳过。背景:taker 秒成交单无 PLACEMENT,旧行为停 SUBMITTED → inflight-check 0 价 inferred fill(见 execution architecture §3.1 #253)|
