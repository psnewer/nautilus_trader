...
- ✅ `test_arb_node.py`:NT exec config 保持启动期 `reconciliation=True`;连续 `open_check_interval_secs=300`(#111:驱动 order liveness 恢复);`position_check_interval_secs=300`(#110:驱动 PM merge/redeem + position liveness);in-flight check 等待阈值为 `10_000ms`、单次查询(`#378`);`timeout_connection=180s`。
