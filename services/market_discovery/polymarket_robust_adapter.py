...
"""
使用 py_clob_client 获取 Polymarket 数据（处理各种返回格式）
"""
...
class PolymarketRobustAdapter:
    """Polymarket 健壮适配器"""
    
    def __init__(self):
        self._log = logging.getLogger('PolymarketRobust')
    ...
    def discover_markets(
        ...
        self._log.info(f'发现 Polymarket 市场 (limit={limit})...')
        
        # 获取市场
        raw_response = self.client.get_markets()
        
        # 解析响应
        markets = self._parse_markets_response(raw_response)
        self._log.info(f'获取到 {len(markets)} 个市场')
        
        if not markets:
            self._log.warning('未获取到市场数据')
            return []
        
        # 过滤活跃市场
        if active_only:
            markets = [m for m in markets if self._is_active(m)]
    ...
    def _parse_markets_response(self, response: Union[list, dict, str]) -> List[dict]:
        """解析市场响应（处理各种格式）"""
        
        # 如果是列表，直接返回
        if isinstance(response, list):
            self._log.debug('响应是列表格式')
            return [m for m in response if isinstance(m, dict)]
        
        # 如果是字典，查找 data 字段
        if isinstance(response, dict):
            self._log.debug('响应是字典格式')
            
            # 尝试不同的键
            for key in ['data', 'markets', 'results']:
                if key in response and isinstance(response[key], list):
                    self._log.debug(f'找到市场列表: {key}')
                    return [m for m in response[key] if isinstance(m, dict)]
            
            # 如果字典本身就是一个市场
            if 'question' in response or 'condition_id' in response:
                return [response]
        
        # 其他情况
        self._log.error(f'未知的响应格式: {type(response)}')
        return []
    
    def _is_active(self, market: dict) -> bool:
        """检查市场是否活跃"""
        return market.get('active', False) and not market.get('closed', False)
    ...
    def _convert_to_standard_event(self, market: dict) -> Optional[StandardEvent]:
        ...
        event = StandardEvent(
            ...
            is_live=False,
            is_active=self._is_active(market),
            metadata={
                'question': question,
...
def test_adapter():
    ...
    print('=' * 70)
    print('测试 Polymarket 健壮适配器')
    print('=' * 70)
    print()
    
    adapter = PolymarketRobustAdapter()
    ...
    print(f'\n✅ 获取到 {len(events)} 个体育事件')
    
    if events:
        print('\n示例事件:')
        for i, event in enumerate(events[:10], 1):
            print(f'\n{i}. {event.event_name[:80]}')
