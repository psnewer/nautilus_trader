# -*- coding: utf-8 -*-
# -------------------------------------------------------------------------------------------------
#  Polymarket 适配器 - 使用 NautilusTrader
# -------------------------------------------------------------------------------------------------

"""
使用 NautilusTrader 的 Polymarket 适配器获取市场数据
"""
...
class PolymarketNautilusAdapter:
    """使用 NautilusTrader 的 Polymarket 适配器"""
    
    def __init__(self):
        self._log = logging.getLogger('PolymarketNautilus')
        ...
        # 创建 instrument provider
        clock = TestClock()
        config = PolymarketDataClientConfig()
        
        self.instrument_provider = PolymarketInstrumentProvider(
...
async def test_adapter():
    ...
    print('=' * 70)
    print('测试 Polymarket NautilusTrader 适配器')
    print('=' * 70)
    print()
    
    adapter = PolymarketNautilusAdapter()
    
    events = await adapter.discover_markets(
        limit=200,
        sports_only=True,
        active_only=True
    )
    ...
    if events:
        ...
        # 保存
        with open('polymarket_nautilus_events.json', 'w', encoding='utf-8') as f:
            json.dump([e.to_dict() for e in events], f, indent=2, ensure_ascii=False)
        
        print(f'\n✅ 已保存到 polymarket_nautilus_events.json')
